...
import type { NodeRef, InequalityData, LatexCmdsAny } from '../../shared_types'
...
import { h } from '../../dom'
import { DOMView, MathCommand, MQSymbol, VanillaSymbol, BinaryOperator, MathBlock, bindVanillaSymbol, bindBinaryOperator } from './core'
...
class DigitGroupingChar extends MQSymbol {
    ...
    siblingDeleted(opts: Options, dir: Direction) {
        this.sharedSiblingMethod(opts, dir)
    }
    siblingCreated(opts: Options, dir: Direction) {
        this.sharedSiblingMethod(opts, dir)
    }

    sharedSiblingMethod(opts: Options, dir: Direction) {
...
LatexCmds.backslash = bindVanillaSymbol('\\backslash ', '\\')
...
LatexCmds['&'] = () =>
    new NonSymbolaSymbol('\\&', h.entityText('&amp;'))
...
LatexCmds.pi = LatexCmds['π'] = () =>
    new NonSymbolaSymbol('\\pi ', h.entityText('&pi;'))
LatexCmds.lambda = () =>
    new NonSymbolaSymbol('\\lambda ', h.entityText('&lambda;'))
...
class PlusMinus extends BinaryOperator {
    ...
    sharedSiblingMethod(_opts?: Options, dir?: Direction) {
...
LatexCmds['±'] =
    ...
    LatexCmds.plusmn =
    LatexCmds.plusminus =
    () => new PlusMinus('\\pm ', h.entityText('&plusmn;'))
LatexCmds.mp =
    LatexCmds.mnplus =
    LatexCmds.minusplus =
    () => new PlusMinus('\\mp ', h.entityText('&#8723;'))
...
setLatexParserNodeConstructors({ Letter, Digit })
